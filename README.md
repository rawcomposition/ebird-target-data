...
```bash
run `caffeinate -i tar -xf ~/Downloads/ebd_relDec-2025.tar -C ~/Downloads`
```

Leave the `ebd_relDec-2025.txt.gz` compressed, DuckDB can read directly from the gzipped file.

## Usage

```bash
python3 build_month_observations.py <species_file.gz> <output.db>
```

For large datasets, use memory and thread options:

```bash
python3 build_month_observations.py ~/Downloads/ebd_relDec-2025.txt.gz ebird.db \
    --memory-limit 24GB \
    --threads 8
```
